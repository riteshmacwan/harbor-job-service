...
export class CommonUtils {
  ...
  async getSenderPhone() {
    ...
    const accountSid = await this.getSecret(
      `${process.env.NODE_ENV ?? "local"}-TWILIO-ACCOUNTSID`
    );
    const authToken = await this.getSecret(
      `${process.env.NODE_ENV ?? "local"}-TWILIO-AUTHTOKEN`
    );
    ...
    try {
      ...
      if (response && response.data && response.data.incoming_phone_numbers) {
        for (const number of response.data.incoming_phone_numbers) {
          ...
          if (
            environment === "staging" &&
            number.phone_number == "+13617333627"
          ) {
            continue;
          }
  ...
  uploadImage = async (body, blobName) => {
    try {
      ...
      const blobServiceClient =
        BlobServiceClient.fromConnectionString(blobSecret);
