...
export const communicationCreate: ValidationChain[] = [
  ...
  check("status")
    ...
    .withMessage(
      "Status must be one of the following ['draft', 'pending_review', 'discarded']."
    ),
  ...
  check("referral_source")
    .optional()
    .isArray()
    .withMessage("Referral source must be an array."),
  check("referral_source.*.id")
    .isInt({ min: 1 })
    .withMessage("Referral source ID must be a positive integer."),
  check("referral_source.*.name")
    .trim()
    .notEmpty()
    .withMessage("Referral Source Name is required."),
  ...
  check("frequency").custom((value, { req }) => {
    ...
    if (value && !["one_time", "recurring"].includes(value)) {
      throw new Error(
        "Type must be one of the following ['one_time', 'recurring']"
      );
    }
    // Return true to indicate the validation succeeded
    return true;
  ...
  check("script_id")
    ...
    .trim()
    .isMongoId()
    .withMessage("Script Id is not valid."),
];
...
export const sendMailValidation: ValidationChain[] = [
   ...
   */
  check("from", "Invalid from mail address")
    .trim()
    .escape()
    .notEmpty()
    .withMessage("From mail address is required.")
    .isString()
    .isEmail()
    .isLength({ max: 100 })
    .withMessage("From mail address must be less then 100.")
    .custom((value, { req }) => {
   ...
   */
  check("to", "Invalid to mail address")
    .notEmpty()
    .withMessage("To mail address is required.")
    .isString()
    .isEmail()
    .isLength({ max: 100 })
    .withMessage("To mail address must be less then 100.")
    .trim()
    .escape(),
   ...
   */
  check("cc", "CC Email must be an array and alteast single element required")
    .optional()
    .isArray({ min: 1 })
    .trim()
    .escape(),
  check("cc.*", "Invalid cc email address")
    .notEmpty()
    .isString()
    .isEmail()
    .isLength({ max: 100 })
    .withMessage("CC mail address must be less then 100.")
    .custom((value, { req }) => {
      if (!emailRegex.test(value)) {
    ...
    .withMessage("Not a valid support email in cc")
    .trim()
    .escape(),
   ...
   */
  check("bcc", "BCC Email must be an array and alteast single element required")
    .optional()
    .isArray({ min: 1 })
    .trim()
    .escape(),
  check("bcc.*", "Invalid bcc email address")
    .notEmpty()
    .isString()
    .isEmail()
    .isLength({ max: 100 })
    ...
    .withMessage("Not a valid support email in bcc")
    .trim()
    .escape(),
  ...
  check("subject", "Invalid subject")
    ...
    .withMessage("Subject must be in between 2 to 100 characters only.")
    .isString()
    .trim()
    .escape(),
];
...
export const sendSMSValidation: ValidationChain[] = [
  ...
  check("type", "Invalid type")
    ...
    .withMessage("Type must be three characters.")
    .isIn(["sms"])
    .trim()
    .escape(),
  ...
  check("media", "Invalid media")
    ...
    .withMessage("Not an valid image")
    .trim()
    .escape(),
];
...
export const sendReplyMailValidation: ValidationChain[] = [
   ...
   */
  check("com_history_id", "Invalid communication history id")
    .notEmpty()
    .withMessage("Communication history id is required.")
    .isString()
    .isMongoId()
    .trim()
    .escape(),
