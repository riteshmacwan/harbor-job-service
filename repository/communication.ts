...
export class CommunicationRepository {
  ...
  async countDocument(condition: object = {}): Promise<number> {
    try {
      ...
      console.log(
        "CommunicationRepository/countDocument Error -->",
        error?.message
      );
      return 0;
    }
  ...
  async findOne(condition: object = {}): Promise<any> {
    try {
      return await Communication.findOne(condition).sort({ token: -1 }).exec();
    } catch (error: any) {
  ...
  async findOneById(id: string): Promise<CommunicationResponse | null> {
    try {
      ...
      console.log(
        "CommunicationRepository/findOneById Error -->",
        error?.message
      );
      return null;
    }
   ...
   */
  async findAllByRecurringStatus(
    condition: object = {}
  ): Promise<CommunicationResponse[] | []> {
    try {
      let data = (await Communication.find(
        condition
      )) as CommunicationResponse[];
      return data;
    } catch (error: any) {
